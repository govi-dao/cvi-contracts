...
const {
  LEVERAGE_TO_THRESHOLD,
  LEVERAGE_TO_MAX,
  LIQUIDATION_MIN_REWARD_PERCENTAGE,
...
const calculateBalanceByPU = (positionUnits, openCVIValue, threshold, leverage = new BN(1)) => {
  const originalBalance = positionUnits
    .mul(openCVIValue)
    .div(CVI_MAX_VALUE)
    .sub(
      positionUnits
...
const testLiquidationReward = async (thresholds, minRewardThreshold, maxRewards) => {
  ...
  for (let i = 0; i < MAX_LEVERAGE; i++) {
    ...
    for (let openCVIValue of OPEN_CVI_VALUES) {
      ...
      // Negative amount
      expect(
        await this.liquidation.getLiquidationReward(threshold, false, positionUnits, openCVIValue, leverage),
      ).to.be.bignumber.equal(minReward)
      expect(
        await this.liquidation.getLiquidationReward(
      ...
      expect(
        await this.liquidation.getLiquidationReward(new BN(0), false, positionUnits, openCVIValue, leverage),
      ).to.be.bignumber.equal(minReward)
      expect(
        await this.liquidation.getLiquidationReward(
          threshold.sub(new BN(1)),
          false,
          positionUnits,
          openCVIValue,
          leverage,
      ...
      expect(
        await this.liquidation.getLiquidationReward(new BN(1), true, positionUnits, openCVIValue, leverage),
      ).to.be.bignumber.equal(minReward)

      // Between minimum and maximum
      if (minReward.eq(maxReward) && !threshold.eq(maxReward)) {
        expect(
          await this.liquidation.getLiquidationReward(minReward, true, positionUnits, openCVIValue, leverage),
        ).to.be.bignumber.equal(minReward)
      } else {
        if (!minReward.eq(threshold)) {
          ...
          expect(
            await this.liquidation.getLiquidationReward(
              minReward.add(new BN(1)),
              true,
              positionUnits,
              openCVIValue,
              leverage,
            ),
          ).to.be.bignumber.equal(minReward.add(new BN(1)))
        }

        expect(
          await this.liquidation.getLiquidationReward(
            maxReward.sub(new BN(1)),
            true,
            positionUnits,
            openCVIValue,
            leverage,
          ),
        ).to.be.bignumber.equal(minReward.eq(maxReward) ? maxReward : maxReward.sub(new BN(1)))

        if (!threshold.eq(maxReward)) {
          expect(
            await this.liquidation.getLiquidationReward(maxReward, true, positionUnits, openCVIValue, leverage),
          ).to.be.bignumber.equal(maxReward)
        }
      }
...
describe('Liquidation', () => {
  ...
  it('detects liquidation properly', async () => {
    ...
    for (let i = 0; i < MAX_LEVERAGE; i++) {
      ...
      for (let openCVIValue of OPEN_CVI_VALUES) {
        ...
        expect(
          await this.liquidation.isLiquidationCandidate(
            balance.add(new BN(1)),
            true,
            positionUnits,
            openCVIValue,
            leverage,
  ...
  it('sets min liquidation reward properly', async () => {
    expect(await this.liquidation.liquidationMinRewardPercent()).to.be.bignumber.equal(
      LIQUIDATION_MIN_REWARD_PERCENTAGE,
    )

    await this.liquidation.setMinLiquidationRewardPercent(LIQUIDATION_MIN_REWARD_PERCENTAGE.div(new BN(2)), {
      from: admin,
    })
    expect(await this.liquidation.liquidationMinRewardPercent()).to.be.bignumber.equal(
      LIQUIDATION_MIN_REWARD_PERCENTAGE.div(new BN(2)),
    )
    await testLiquidationReward(
      LEVERAGE_TO_THRESHOLD,
      LIQUIDATION_MIN_REWARD_PERCENTAGE.div(new BN(2)),
      LEVERAGE_TO_MAX,
    )
  ...
  it('reverts when setting liquidation threshold to less than max (but not equal)', async () => {
    for (let i = 0; i < MAX_LEVERAGE; i++) {
      ...
      await expectRevert(
        this.liquidation.setMinLiquidationThresholdPercents(newThresholds, { from: admin }),
        'Threshold less than some max',
      )
    }
  ...
  it('reverts when setting liquidation max to less than min (but not equal)', async () => {
    for (let i = 0; i < MAX_LEVERAGE; i++) {
      ...
      await expectRevert(
        this.liquidation.setMaxLiquidationRewardPercents(newMaxRewards, { from: admin }),
        'Some max less than min',
      )
    }
  ...
  it('reverts when setting liquidation max to more than threshold (but not equal)', async () => {
    for (let i = 0; i < MAX_LEVERAGE; i++) {
      ...
      await expectRevert(
        this.liquidation.setMaxLiquidationRewardPercents(newMaxRewards, { from: admin }),
        'Some max greater than threshold',
      )
    }
  ...
  it('reverts when setting liquidation min to more than max (but not equal)', async () => {
    await expectRevert(
      this.liquidation.setMinLiquidationRewardPercent(LEVERAGE_TO_MAX[0].add(new BN(1)), { from: admin }),
      'Min greater than some max',
    )
  ...
  it('reverts when not called by owner', async () => {
    await expectRevert(
      this.liquidation.setMinLiquidationRewardPercent(LIQUIDATION_MIN_REWARD_PERCENTAGE, { from: alice }),
      'Ownable: caller is not the owner',
    )
    await expectRevert(
      this.liquidation.setMaxLiquidationRewardPercents(LEVERAGE_TO_MAX, { from: alice }),
      'Ownable: caller is not the owner',
    )
    await expectRevert(
      this.liquidation.setMinLiquidationThresholdPercents(LEVERAGE_TO_THRESHOLD, { from: alice }),
      'Ownable: caller is not the owner',
    )
  })
})
