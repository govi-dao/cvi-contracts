...
const beforeEachMigrator = async isETH => {
  ...
  this.fakeExchange = await FakeExchange.new(ZERO_ADDRESS, { from: admin })
  await this.fakeExchange.setExchangeRate(USDT_TO_USDC_RATE, this.newToken.address, { from: admin })
  ...
  await setMigratorAsLiquidityProvider()
}

const setMigratorAsLiquidityProvider = async () => {
  await this.oldContracts.platform.setAddressSpecificParameters(this.migrator.address, true, false, false, true, {
    from: admin,
  })
  await this.newContracts.platform.setAddressSpecificParameters(this.migrator.address, true, false, false, true, {
    from: admin,
  })
  await this.newSameTokenContracts.platform.setAddressSpecificParameters(
...
const setMigratorTests = () => {
  ...
  it('sets uniswap pair properly', async () => {
    const badFakeExchange = await FakeExchange.new(ZERO_ADDRESS, { from: admin })
    await badFakeExchange.setExchangeRate(USDT_TO_USDC_RATE.mul(new BN(2)), this.newToken.address, { from: admin })
    this.migrator = await PlatformMigrator.new(
      this.rewardToken.address,
      this.token.address,
      this.oldContracts.platform.address,
  ...
  it('withdraws rewards', async () => {
    ...
    expect(ownerRewardsAfterWithdrawingAll.toString()).to.equal(
      ownerRewardsBeforeWithdrawingAll.add(balanceOfRewards).toString(),
    )
  })
  ...
  it('sets slippage percent properly', async () => {
    const prevSlippagePercent = await this.fakeExchange.slippagePercents(this.newToken.address)
    const maxPercentage = await this.fakeExchange.MAX_PERCENTAGE()

    expect(prevSlippagePercent).to.be.bignumber.equal(new BN(0))
    await this.fakeExchange.setSlippagePercent(1000, this.newToken.address, { from: admin })

    const newSlippagePercent = await this.fakeExchange.slippagePercents(this.newToken.address)
    expect(prevSlippagePercent).to.be.bignumber.not.equal(newSlippagePercent)

    await expectRevert(testMigration(this.newContracts, true), 'Fake Uniswap: output below min')
  })
  ...
  it('migrates LP tokens when min amount out is equal to amount out with slippage', async () => {
    ...
    await this.fakeExchange.setSlippagePercent(1000, this.newToken.address, { from: admin }) // slippage is 90%, exchange rate is 90%, we'll be getting 81% of amount back
